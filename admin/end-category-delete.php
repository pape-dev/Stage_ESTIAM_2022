...
if (!isset($_REQUEST['id'])) {
	...
	// Vérification si l'identfiant est valide ou non
...
for ($i = 0; $i < count($p_ids); $i++) {
	...
	// Obtenir une pièce d'identité avec photo 
	...
	// Obtenir une autre pièce d'identité avec photor
	...
	// Supprimer de la table photo
	$statement = $pdo->prepare("DELETE FROM tbl_product WHERE p_id=?");
	$statement->execute(array($p_ids[$i]));

	// Supprimer de la tbl_product_photo
	$statement = $pdo->prepare("DELETE FROM tbl_product_photo WHERE p_id=?");
	$statement->execute(array($p_ids[$i]));

	// Supprimer de la tbl_product_size
	$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE p_id=?");
	$statement->execute(array($p_ids[$i]));

	// Supprimer de latbl_product_color
	$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE p_id=?");
	$statement->execute(array($p_ids[$i]));

	// Supprimer de la tbl_rating
...
// Supprimer de la tbl_end_category
